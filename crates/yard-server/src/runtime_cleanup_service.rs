...
impl RuntimeCleanupService {
    ...
    async fn process(
        ...
        let jobs = self
            .store
            .list_pending_runtime_cleanups(command_id, RUNTIME_CLEANUP_BATCH_SIZE)
            .await?;
        ...
        for job in jobs {
            match self.runtime.retire_runtime(retirement_request(&job)).await {
                Ok(()) => {
                    self.store.succeed_runtime_cleanup(&job.id).await?;
                    report.succeeded += 1;
                }
                Err(error) => {
                    self.store
                        .fail_runtime_cleanup(&job.id, &error.to_string())
                        .await?;
                    report.failed += 1;
                    warn!(
                        cleanup_id = %job.id,
